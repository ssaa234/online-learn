...
import com.ape.apesystem.service.ApeUserService;  // 添加导入
...
public class ApeArticleController {
    ...
    @Autowired
    private ApeUserService apeUserService;  // 添加用户服务
    ...
    public Result getApeArticlePage(@RequestBody ApeArticle apeArticle) {
        ...
        queryWrapper.lambda()
                ...
                .eq(StringUtils.isNotBlank(apeArticle.getTaskId()), ApeArticle::getTaskId, apeArticle.getTaskId());
        ...
        // ========== 根据 user_id 获取用户头像 ==========
        for (ApeArticle article : apeArticlePage.getRecords()) {
            if (StringUtils.isNotBlank(article.getUserId())) {
                ApeUser user = apeUserService.getById(article.getUserId());
                if (user != null) {
    ...
    public Result getIndexArticleList() {
        ...
        // 首页推荐笔记也获取头像
        for (ApeArticle article : articleList) {
            if (StringUtils.isNotBlank(article.getUserId())) {
                ApeUser user = apeUserService.getById(article.getUserId());
                if (user != null) {
                    article.setAvatar(user.getAvatar());
    ...
    public Result getApeArticleById(@RequestParam("id")String id) {
        ...
        // 获取用户头像
        if (apeArticle != null && StringUtils.isNotBlank(apeArticle.getUserId())) {
            ApeUser user = apeUserService.getById(apeArticle.getUserId());
            if (user != null) {
                apeArticle.setAvatar(user.getAvatar());
            }
        }
