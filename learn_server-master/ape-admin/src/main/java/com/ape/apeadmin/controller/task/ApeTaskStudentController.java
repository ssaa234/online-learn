...
public class ApeTaskStudentController {
    ...
    public Result getApeMyTaskPage(@RequestBody ApeTaskStudent apeTaskStudent) {
        ...
        // 根据 taskId 获取完整的课程信息
        for (ApeTaskStudent taskStudent : apeTaskStudentPage.getRecords()) {
            if (StringUtils.isNotBlank(taskStudent.getTaskId())) {
                ...
                if (task != null) {
                    ...
                    taskStudent.setGradeLevel(task.getGradeLevel());
                }
            }

            // 计算学习进度
            int progress = calculateProgress(taskStudent.getTaskId(), userInfo.getId());
            taskStudent.setProgress(progress);
        }
     ...
     * @return 进度百分比
     */
    private int calculateProgress(String taskId, String userId) {
        try {
            // 获取课程所有章节数量
            QueryWrapper<ApeChapter> chapterWrapper = new QueryWrapper<>();
            chapterWrapper.lambda().eq(ApeChapter::getTaskId, taskId);
            int totalChapters = apeChapterService.count(chapterWrapper);
            ...
            // 获取用户已观看的章节数量
            QueryWrapper<ApeChapterVideo> videoWrapper = new QueryWrapper<>();
            videoWrapper.lambda()
                    .eq(ApeChapterVideo::getUserId, userId)
                    .inSql(ApeChapterVideo::getChapterId, "SELECT id FROM ape_chapter WHERE task_id = '" + taskId + "'");
            int watchedChapters = apeChapterVideoService.count(videoWrapper);

            // 进度 = 已观看章节数 / 总章节数 * 100
            return (int) ((double) watchedChapters / totalChapters * 100);
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    public Result editApeTaskStudent(@RequestBody ApeTaskStudent apeTaskStudent) {
        ...
        if (apeTaskStudent.getState() == 0) {
            task.setNum((task.getNum() == null ? 0 : task.getNum()) + 1);
        } else if (student.getState() == 0 && apeTaskStudent.getState() == 1) {
            task.setNum((task.getNum() == null ? 0 : task.getNum()) - 1);
        }
