...
import {editApeTask,getApeTaskById,getUserListByType,getApeClassificationList} from '../../../api/api'
export default {
  mixins: [mixin],
  data() {
    return{
      ...
      showClassification: false, // 是否显示所属分类
      gradeOptions: [],         // 年级选项
      rules: {
        proportion: [
  ...
  methods: {
    ...
    // 根据学段设置页面显示状态（用于编辑时回显）
    setFormDisplayByGradeLevel(gradeLevel) {
      if (gradeLevel === '其他') {
        this.showGrade = false
        this.showTextbook = false
        this.showClassification = true
        ...
        this.showClassification = false
        this.rules.classification[0].required = false
        this.setGradeOptions(gradeLevel)
      } else {
        this.showGrade = false
        this.showTextbook = false
        this.showClassification = false
      ...
      if(newVal) {
        getApeTaskById({id:newVal}).then(res => {
          if(res.code == 1000) {
            ...
            // 根据学段设置显示状态
            this.$nextTick(() => {
              this.setFormDisplayByGradeLevel(this.form.gradeLevel)
            })
