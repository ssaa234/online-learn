    ...
    <div class="search-table">
      ...
      <div class="table">
        ...
        <el-table
          ...
          <el-table-column prop="taskName" label="课程名称" width="300"></el-table-column>
          <el-table-column prop="name" label="章节名称" width="350"></el-table-column>
          <el-table-column prop="createTime" label="创建时间" width="200"></el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="warning" @click="toStudyInfo(scope.row.id, scope.row.name)">学习情况</el-button>
              <!-- 修改：点击作业时传递章节ID和章节名称 -->
              <el-button size="mini" type="primary" @click="toHomework(scope.row.id, scope.row.name, scope.row.taskId)">作业</el-button>
...
export default {
  ...
  methods: {
    ...
    toHomework(chapterId, chapterName, taskId) {
      // 对章节名称进行编码，避免URL中出现特殊字符
      const encodedName = encodeURIComponent(chapterName)
      var param = {
        "name": "章节作业",
        "url": "/homework?chapterId=" + chapterId + "&chapterName=" + encodedName
      }
      console.log('跳转到作业页面 - 章节ID:', chapterId, '章节名称:', chapterName)
      this.$store.commit('menu/addActiveMenu', param)
      this.$router.push("/homework?chapterId=" + chapterId + "&chapterName=" + encodedName)
      this.$store.commit('menu/setActiveMenu', "/homework?chapterId=" + chapterId + "&chapterName=" + encodedName)
    },
    searchPage() {
